# Create datanode.config with an empty array
$dataNodeConfig = @{
    datanodes = @()
} | ConvertTo-Json

$dataNodeConfig | Out-File "datanode.config"

# Create namenode.config with "email,password" text
echo "email,password" | Out-File "namenode.config"

# Create gaps.json with the same content as datanode.config but with "gaps" as the key
$gapsConfig = @{
    gaps = @()
} | ConvertTo-Json

$gapsConfig | Out-File "gaps.json"

# Create genNewNode.py with the provided Python script content
$pythonScriptContent = @'
import json
import requests
import subprocess
...
$pythonScriptContent | Out-File "genNewNode.py" -Encoding utf8
